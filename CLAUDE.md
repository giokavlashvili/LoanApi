...
- **Logging**: NLog via `nlog.config`, using the connection string pushed into `GlobalDiagnosticsContext` by `AddNlog`. Targets are wrapped in an `AsyncWrapper` (`<targets async="true">`) so SQL inserts never run on the request thread; `throwConfigExceptions="true"` means an invalid `nlog.config` fails at startup rather than silently logging nothing.
  ...
  - **Log columns come from three sources**, all mapped in `nlog.config`: `${aspnet-*}` ambient renderers (correlation id, url, method, ip, username — available on *any* event raised during a request), `${event-properties:item=X}` from message-template holes (status, duration, user id, channel), and `${scopeproperty:item=X}` from `ILogger.BeginScope` for the two large bodies, which keeps them out of the rendered `Message`. Property names are declared in `Application/Common/Logging/LogProperties.cs` and must stay in sync with `Domain/Entities/Log.cs` and the INSERT in `nlog.config`. Because these are plain structured properties, adding a Seq (or other structured) target later is a config change with no code change.
  - **Routing rules**: `LoggingMiddleware` writes to the database at `Info` (one row per request); every other logger needs `Warning`+ to get in. `Channel` distinguishes `Request` rows from `Api` rows.
